...
!!! tip "Мнемоника"
    `save` пишет в файл -> нужен режим `"wb"` (write binary). `load` читает из файла -> нужен режим `"rb"` (read binary). Save = write = "wb", load = read = "rb".

Если перепутать (`save` в `"rb"` или `load` в `"wb"`) - либо упадет сразу с `io.UnsupportedOperation`, либо (что хуже) `load()` в режиме `"wb"` тихо затрет файл перед чтением, и потом `pickle.load` упадет на пустом файле.
