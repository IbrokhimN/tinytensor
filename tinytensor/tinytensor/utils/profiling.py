...
def benchmark(model, x, runs=100, warmup=10, batch_size=None):
    ...
    # прогрев: первые прогоны часто медленнее (кэши, ленивая инициализация)
...
def _run_once(model, x):
    # один forward-прогон через модель
...
def model_size(model, filepath=None):
    # реальный размер модели на диске в кб. если filepath не дан - сохраняем во временный.
    # для квантованных моделей это честнее чем count_params*4 (int8 = 1 байт)
    import os
...
def flops(model, input_shape):
    # число операций (MACs) на один объект. input_shape с батчем (N,C,H,W) или (N,features).
    # считаем по Conv2d и Linear, прогоняя вход чтобы знать формы выходов.
    from tinytensor.core.tensor import Tensor
    ...
    if layers is None:
        # не Sequential - разворачиваем подмодули
